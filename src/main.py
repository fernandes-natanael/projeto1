...
def list_crops():
    ...
    for crop in crops:
        ...
        print(
            f"== idx: {crop.id},  Tipo: {crop.type}, Tipo de Area: {crop.area_type}, Área: {crop.area:.2f} m², Total de Insumos: {crop.input_management:.2f} {medida_insumo}"
        )
...
if __name__ == "__main__":
    ...
    while not exit:
        ...
        if option == "1":
            ...
            update_csv = (
                input("Deseja atualizar csv com dados atuais s/n\n:").strip().lower()
            )
            if update_csv == "s":
                update_all_crops_in_csv(crops)
